...
export function step(bs: Brick[], ps: Set<string>): Set<number> {
    ...
    for (let i = 0; i < bs.length; i++) {
        ...
        if (b.cubes
            .filter(c => c.z === minZ)
            .every(c => c.z > 1 && !ps.has(key({ x: c.x, y: c.y, z: c.z - 1 })))
        ) {
            b.cubes.forEach(c => {
                ps.delete(key(c))
                c.z--
...
export function mkSupportMap(bs: Brick[]): Map<number, Set<number>> {
